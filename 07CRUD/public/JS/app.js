...
async function cargarUsuarios(){
    try {
        //vamos a utilizar una funcion llamada fetch la cual realiza una petición http por defecto mediante el metodo get, mientras que al ser una funcion asincrona, espera (await) que la peticion se complete antes de continuar
        const respuesta = await fetch(API_URL);

        //si responde ok el codigo nos da un estado de 200 a 299
        if(respuesta.ok){
            //hay un error
